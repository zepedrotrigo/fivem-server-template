...
function gsrcheck(source, identifier)
	...
	if Config.UseCharName then
		...
		if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'O teste a '..fullName..' José Trigo deu positivo', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'O teste a '..fullName..' deu negativo', length = 5000 })
    	end
	else
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = 'O teste deu positivo', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = 'O teste deu negativo', length = 5000 })
    	end
	end
end
