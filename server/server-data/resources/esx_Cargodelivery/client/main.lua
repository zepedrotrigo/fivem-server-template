	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Pressiona ~INPUT_CONTEXT~ para interagir com ~y~Vendedor")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Volta para dentro do veiculo!", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Morreste!')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed('A mercadoria ficou danificada!')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Acabou o tempo!')
						end
...
function CargoMenu()
    ...
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Comprar mercadoria - $" .. Config.Scenarios[i].CargoCost, "PurchaseCargo", i)
   	end
...
function AlertThePolice()
	...
	if event_is_running then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":anon: Vi uma carrinha preta suspeita com a matrícula [ " .. vehicle_plate .. " ] pode ser que leve algo de valioso!", PlayerCoords, {
    	...
    	for i = 1, #Config.AlertExtraSocieties do
    		...
    		if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    			...
    			TriggerServerEvent('esx_addons_gcphone:startCall', Config.AlertExtraSocieties[i], ":anon: Vi uma carrinha preta suspeita com a matrícula [ " .. vehicle_plate .. " ] pode ser que leve algo de valioso!", PlayerCoords, {
				PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
				})
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true then
		...
		drawNotification("Já está a decorrer um transporte.")
		goto done
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				drawNotification("Compraste a mercadoria com sucesso.")
			...
			drawNotification("São precisos ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " policias ~w~de serviço.")
