...
local Positions = {
	...
	['Sell'] = { ['hint'] = '[E] Sell', ['x'] = -68.76375732422, ['y'] = 6253.6833203125, ['z'] = 31.475671768188 },
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Esfolar veado', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Precisas de equipar a tua faca!')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Esfolaste o veado e conseguiste ' ..AnimalWeight.. 'kg')
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(-769.06, 5595.9, 33.49)
	...
	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName("Caça")
