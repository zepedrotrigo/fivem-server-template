...
function UpdatePlayerTable(connectedPlayers)
	...
	local ems, police, taxi, mechanic, cardealer, unemployed, players, unicorn = 0, 0, 0, 0, 0, 0, 0, 0
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, taxi = taxi, mechanic = mechanic, cardealer = cardealer, unemployed = unemployed, player_count = players, unicorn = unicorn}
	})
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['PAGEUP']) or IsControlJustReleased(0, Keys['7']) and IsInputDisabled(0) then
			ToggleScoreBoard()
			Citizen.Wait(200)

		-- D-pad up on controllers works, too!
		elseif IsControlJustReleased(0, Keys["PAGEUP"]) and not IsInputDisabled(0) then
			ToggleScoreBoard()
			Citizen.Wait(200)
		end
	end
