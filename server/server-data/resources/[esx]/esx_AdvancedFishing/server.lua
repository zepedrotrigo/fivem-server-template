...
ESX.RegisterUsableItem('turtlebait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocaste a isca de tartarugas na cana de pesca")
	else
		TriggerClientEvent('fishing:message', _source, "~r~Não tens uma cana de pesca!")
	end
...
ESX.RegisterUsableItem('fishbait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocaste a isca de peixes na cana de pesca")
...
ESX.RegisterUsableItem('turtle', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocaste a carne de tartaruga na cana de pesca")
	else
		TriggerClientEvent('fishing:message', _source, "~r~Não tens uma cana de pesca!")
	end
	...
	if bait == "turtle" then
		if rnd >= 78 then
			if rnd >= 94 then
				...
				TriggerClientEvent('fishing:message', _source, "~r~Era enorme e partiu a tua cana de pesca!")
				...
				if xPlayer.getInventoryItem('turtle').count > 4 then
					TriggerClientEvent('fishing:message', _source, "~r~Não consegues transportar mais tartarugas")
				else
					TriggerClientEvent('fishing:message', _source, "~g~Apanhaste uma tartaruga\n~r~é ilegal pescar esta espécie")
			...
			if rnd >= 75 then
				if xPlayer.getInventoryItem('fish').count > 100 then
					TriggerClientEvent('fishing:message', _source, "~r~Não consegues transportar mais peixes!")
				else
					weight = math.random(4,9)
					TriggerClientEvent('fishing:message', _source, "~g~Apanhaste um peixe: ~y~~h~" .. weight .. "kg")
		...
		if bait == "shark" then
			if rnd >= 82 then
						...
						if rnd >= 91 then
							TriggerClientEvent('fishing:setbait', _source, "none")
							TriggerClientEvent('fishing:message', _source, "~r~Era enorme e partiu te a cana de pesca!")
							TriggerClientEvent('fishing:break', _source)
							xPlayer.removeInventoryItem('fishingrod', 1)
						else
							if xPlayer.getInventoryItem('shark').count > 0  then
									TriggerClientEvent('fishing:setbait', _source, "none")
									TriggerClientEvent('fishing:message', _source, "~r~Não consegues transportar mais tubarões!")
							else
									TriggerClientEvent('fishing:message', _source, "~g~Apanhaste um tubarão!\n~r~é ilegal pescar esta espécie")
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			if item == "fish" then
					local FishQuantity = xPlayer.getInventoryItem('fish').count
						...
						TriggerClientEvent('esx:showNotification', source, '~r~Não tens peixes suficientes!~s~')			
					else   
			...
			if item == "turtle" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~Não tens tartarugas suficientes!~s~')			
				else   
					xPlayer.removeInventoryItem('turtle', 1)
			...
			if item == "shark" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~Não tens tubarões suficientes!~s~')			
				else   
