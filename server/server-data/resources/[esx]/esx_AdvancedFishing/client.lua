...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Venda de peixe")
...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Dealer de Tartarugas")
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Dealer de Tubarão")
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Aluguer de barco")
		...
		if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~O peixe escapou!")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~O peixe mordeu a isca \n ~h~Pressiona " .. correct .. " para o apanhar")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~Não podes pescar enquanto estás dentro de um veículo")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			...
			ESX.ShowNotification("~y~Ainda estás muito perto da costa!")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Pressiona E para alugar barco')
...
function OpenBoatsMenu(x, y , z)
	...
	local elements = {}
		...
		table.insert(elements, {label = '<span style="color:green;">Dinghy</span> <span style="color:red;">20$</span>', value = 'boat'})
		table.insert(elements, {label = '<span style="color:green;">Suntrap</span> <span style="color:red;">40$</span>', value = 'boat6'}) 
		table.insert(elements, {label = '<span style="color:green;">Jetmax</span> <span style="color:red;">80$</span>', value = 'boat5'}) 	
		table.insert(elements, {label = '<span style="color:green;">Toro</span> <span style="color:red;">100$</span>', value = 'boat2'}) 
		table.insert(elements, {label = '<span style="color:green;">Marquis</span> <span style="color:red;">120$</span>', value = 'boat3'}) 
		table.insert(elements, {label = '<span style="color:green;">Tug boat</span> <span style="color:red;">140$</span>', value = 'boat4'})
	...
	if data.current.value == 'boat' then
		...
		TriggerServerEvent("fishing:lowmoney", 20) 
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 20)
	...
	if data.current.value == 'boat2' then
		...
		TriggerServerEvent("fishing:lowmoney", 40) 
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 40)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerServerEvent("fishing:lowmoney", 60) 
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 60)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerServerEvent("fishing:lowmoney", 80) 
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 80)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 100)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerServerEvent("fishing:lowmoney", 120) 
		TriggerEvent("chatMessage", 'Alugaste um barco por', {255,0,255}, '$' .. 120)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Tiraste um barco')
		SetPedCoordsKeepVehicle(ped, x, y , z)
