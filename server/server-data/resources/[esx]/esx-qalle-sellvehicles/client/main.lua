...
function LoadSellPlace()
	Citizen.CreateThread(function()
		...
		AddTextComponentString("Concessionária de Usados")
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "[E] Abrir menu", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							ESX.ShowNotification("Tens que estar dentro de um ~g~veiculo")
						end
					end
				end
			end
					...
					if dstCheck <= 2.0 then
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								ESX.ShowNotification("Tens que estar dentro de um ~g~veiculo~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			...
			table.insert(elements, { ["label"] = "Colocar preço - :-", ["value"] = "price" })
		end
	else
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("Colocaste o ~g~veiculo~s~ à venda - " .. price .. " :-")
					menu.close()
				else
					ESX.ShowNotification("Tens que ser o ~r~dono~s~ do ~g~veiculo!~s~ / já está" .. #Config.VehiclePositions .. " à venda!")
				end
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("~g~Compraste~s~ o veiculo por " .. price .. " :-")
					menu.close()
				else
					ESX.ShowNotification("Não tens ~r~dinheiro~s~ suficiente, faltam " .. price - totalMoney .. " :-")
				end
		...
		elseif action == "remove" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("~g~Removeste~s~ o veiculo")
					menu.close()
				end
