...
AddEventHandler('esx_heligarage:hasEnteredMarker', function(zone)
	...
	if PlayerData.job.name == 'police' then
		...
		if zone == 'PoliceGet' then
			CurrentActionMsg  = 'Pressiona ~INPUT_CONTEXT~ para tirar ~o~helicoptero'
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction == ACTION_GET_POLICEHELI then
			ESX.ShowNotification('~o~Helicoptero~w~ a caminho..')
			Citizen.Wait(6000)
		...
		if CurrentAction == ACTION_REMOVE_POLICEHELI then
			ESX.ShowNotification('A remover ~o~Helicoptero')
			Citizen.Wait(4500)
		...
		if CurrentAction == ACTION_REMOVE_AMBULANCEHELI then
			ESX.ShowNotification('A remover ~o~Helicopter')
			Citizen.Wait(4500)
