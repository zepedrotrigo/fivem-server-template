...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(isNear(tenue)) then
				Info("Pressione ~ g ~ E ~ w ~ para mudar.")
		...
		if(isNear(gamestart)) then
				Info("Pressione ~ g ~ E ~ w ~ para redefinir a pontuação para zero.")
				if(IsControlJustPressed(1, 38)) then
					debutpartie()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, home.x, home.y, home.z) < 2.0) then
				Info("Pressione ~ g ~ E ~ w ~ para sair do paintball.")
				if(IsControlJustPressed(1, 38)) then
					allerjouer()
				end
		end
...
function OuvrirVestiaire()
  ...
  local elements = {
    ...
    table.insert(elements, {label = "Equipamento da equipe 1", value = 'tenue_equipe'})
	table.insert(elements, {label = "Equipamento da equipe 2", value = 'tenue_equipedeux'})
...
function debutpartie()
	...
	if(distancenotification(millieu)) then
		...
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~ A partida ~r~~h~ começa !')
	end
end
...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("~r~ Bom Jogo !!")
		PushScaleformMovieFunctionParameterString("Espero que as armas estejam no inventario !")
