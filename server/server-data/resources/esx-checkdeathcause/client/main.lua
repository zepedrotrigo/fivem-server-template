  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Pressiona [~g~E~s~] para verificar causa da inconsciência', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Parece que o dano ocorreu aqui', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Tentar identificar causa da inconsciência', value = 'deathcause'})
	  table.insert(elements, {label = 'Tentar identificar onde ocorreu o dano', value = 'damage'})
	  ...
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Não conseguiste identificar onde ocorreu o dano')
			  end
		  end
